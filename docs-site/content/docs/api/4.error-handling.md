...
Errors carrying a `.status` property are serialised the same way, using that status code. Any other (unhandled) `Error` is **masked**: its real message is logged server-side (log level 3) and the client receives a generic `500` response — `{ "statusCode": 500, "error": "Internal Server Error", "message": "Internal Server Error" }` — so internal details (stack traces, database driver messages, file paths) are never leaked. Always throw an `IOServerError` when you want a specific message and status code to reach the client.
